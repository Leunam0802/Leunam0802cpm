...
def check_exit():
    ...
    if answ.lower() == "y":
        print(
            Colorate.Horizontal(
                Colors.rainbow,
                f"Gracias por usar nuestra herramienta, únete a nuestro canal de telegram: @{__CHANNEL_USERNAME__}.",
            )
        )
        sys.exit(0)
...
def banner(console):
    ...
    print(
        Colorate.Horizontal(
            Colors.rainbow,
            "\t         POR FAVOR CIERRA SESIÓN EN CPM ANTES DE USAR ESTA HERRAMIENTA",
        )
    )
    print(
        Colorate.Horizontal(
            Colors.rainbow,
            "    COMPARTIR LA CLAVE DE ACCESO NO ESTÁ PERMITIDO Y SERÁ BLOQUEADO",
        )
    )
    print(
        Colorate.Horizontal(
            Colors.rainbow,
            f" ‌           Telegram: @{__CHANNEL_USERNAME__} | {__GROUP_USERNAME__}",
        )
    )
    ...
    try:
        ...
        if response.get("ok"):
            ...
            if (
                ...
                print(
                    Colorate.Horizontal(
                        Colors.rainbow, "==========[ DETALLES DEL JUGADOR ]=========="
                    )
                )
                print(
                    Colorate.Horizontal(
                        Colors.rainbow,
                        f'Nombre   : {(data.get("Name") if "Name" in data else "INDEFINIDO")}.',
                    )
                )
                print(
                    Colorate.Horizontal(
                        Colors.rainbow, f'LocalID  : {data.get("localID")}.'
                    )
                )
                print(
                    Colorate.Horizontal(
                        Colors.rainbow, f'Dinero   : {data.get("money")}.'
                    )
                )
                print(
                    Colorate.Horizontal(
                        Colors.rainbow, f'Monedas  : {data.get("coin")}.'
                    )
                )
                return True
            else:
                print(
                    Colorate.Horizontal(
                        Colors.rainbow,
                        "! ERROR: Las cuentas nuevas deben iniciar sesión en el juego al menos una vez !",
                    )
                )
                sleep(2)
                return False
            ...
            print(
                Colorate.Horizontal(
                    Colors.rainbow,
                    f'! ERROR: {response.get("message", "Sesión no válida")} !',
                )
            )
            sleep(2)
            return False
...
def load_key_data(cpm, console):
    ...
    try:
        ...
        if data:
            print(
                Colorate.Horizontal(
                    Colors.rainbow, "========[ DETALLES DE CLAVE DE ACCESO ]========"
                )
            )
            print(
                Colorate.Horizontal(
                    Colors.rainbow,
                    f'Clave de acceso : {data.get("access_key", "No disponible")}.',
                )
            )
            print(
                Colorate.Horizontal(
                    Colors.rainbow,
                    f'ID de Telegram  : {data.get("telegram_id", "No disponible")}.',
                )
            )
            print(
                Colorate.Horizontal(
                    Colors.rainbow,
                    f'Saldo $         : {(data.get("coins", "0") if not data.get("is_unlimited", False) else "Ilimitado")}.',
                )
            )
            return True
        else:
            print(
                Colorate.Horizontal(
                    Colors.rainbow,
                    "! ERROR: No se pudieron obtener datos de la clave !",
                )
            )
            sleep(2)
            return False
    ...
    while True:
        ...
        if not value or value.isspace():
            print(
                Colorate.Horizontal(
                    Colors.rainbow,
                    f"{tag} no puede estar vacío o solo espacios. Por favor intente nuevamente.",
                )
            )
        else:
            return value
...
def load_client_details(console):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                Colorate.Horizontal(
                    Colors.rainbow, "=============[ UBICACIÓN ]============="
                )
            )
            print(
                Colorate.Horizontal(
                    Colors.rainbow,
                    f'Dirección IP : {data.get("query", "No disponible")}.',
                )
            )
            print(
                Colorate.Horizontal(
                    Colors.rainbow,
                    f'Ubicación    : {data.get("city", "N/A")} {data.get("regionName", "N/A")} {data.get("countryCode", "N/A")}.',
                )
            )
            print(
                Colorate.Horizontal(
                    Colors.rainbow,
                    f'País         : {data.get("country", "N/A")} {data.get("zip", "N/A")}.',
                )
            )
            print(
                Colorate.Horizontal(
            ...
            print(
                Colorate.Horizontal(Colors.rainbow, "No se pudo detectar la ubicación.")
            )
            print(
                Colorate.Horizontal(
                    Colors.rainbow, "===============[ MENÚ ]==============="
...
def mostrar_menu_servicios():
    ...
    print(
        Colorate.Horizontal(Colors.rainbow, "===============[ 𝐋𝐄𝐔𝐍𝐀𝐌 ]===============")
    )


service = IntPrompt.ask(
    f"[bold][?] Selecciona un servicio [red][1-{choices[-1]} o 0][/red][/bold]",
    choices=choices,
    show_choices=False,
)
...
if service == 0:  # Salir
    ...
    console.print(
        "[bold red][!] Nota:[/bold red]: si el rango de rey no aparece en el juego, ciérralo y ábrelo varias veces.",
        end=None,
    )
    console.print(
        "[bold red][!] Nota:[/bold red]: por favor no hagas Rango Rey en la misma cuenta dos veces.",
        end=None,
    )
    sleep(2)
    ...
    print(
        Colorate.Horizontal(
            Colors.rainbow, "[!] Después de eliminar tu cuenta no hay vuelta atrás !!."
        )
    )
    answ = Prompt.ask(
        "[?] ¿Quieres eliminar esta cuenta?!", choices=["y", "n"], default="n"
    )
    if answ == "y":
        cpm.delete()
        print(Colorate.Horizontal(Colors.rainbow, "ÉXITO"))
    ...
    acc2_email = prompt_valid_value(
        "[?] Correo electrónico de la cuenta", "Correo electrónico", password=False
    )
    acc2_password = prompt_valid_value(
        "[?] Contraseña de la cuenta", "Contraseña", password=False
    )
    ...
    if status == 0:
        ...
        print(
            Colorate.Horizontal(
                Colors.rainbow, f"INFO: Para modificar esta cuenta con CPMElsedev."
            )
        )
        print(
            Colorate.Horizontal(
                Colors.rainbow, "debes iniciar sesión en el juego con esta cuenta."
            )
        )
        sleep(2)
        continue
        ...
        print(Colorate.Horizontal(Colors.rainbow, "FALLÓ."))
        print(
            Colorate.Horizontal(Colors.rainbow, "¡Este correo electrónico ya existe!.")
        )
        sleep(2)
        continue
    else:
    ...
    console.print(
        "[!] Nota: esta función tarda un tiempo en completarse, por favor no canceles.",
        end=None,
    )
    ...
    print(
        Colorate.Horizontal(Colors.rainbow, "[!] Ingresa cuántas carreras has ganado.")
    )
    amount = IntPrompt.ask("[?] Cantidad")
    ...
    if (
        ...
        if cpm.set_player_wins(amount):
            ...
            print(
                Colorate.Horizontal(
                    Colors.rainbow, "[!] Por favor usa valores válidos."
                )
            )
            sleep(2)
            continue
    ...
    print(
        Colorate.Horizontal(Colors.rainbow, "[!] Ingresa cuántas carreras has perdido.")
    )
    amount = IntPrompt.ask("[?] Cantidad")
    console.print("[%] Cambiando tus datos: ", end=None)
    if (
    ...
    print(
        Colorate.Horizontal(
            Colors.rainbow, "[!] Por favor ingresa los detalles de la cuenta."
        )
    )
    ...
    if cpm.account_clone(to_email, to_password):
        ...
        print(
            Colorate.Horizontal(
                Colors.rainbow,
                "[!] LA CUENTA RECEPTORA ES GMAIL, LA CONTRASEÑA NO ES VÁLIDA O ESA CUENTA NO ESTÁ REGISTRADA.",
            )
        )
        sleep(2)
        continue
    ...
    try:
        ...
        print(
            Colorate.Horizontal(
                Colors.rainbow, "VALOR INVÁLIDO, POR FAVOR INGRESA UN NÚMERO"
            )
        )
        sleep(2)
        continue
    ...
    while True:
        try:
            custom_input = Prompt.ask(
                "[pink][?] INGRESA PORCENTAJE DE INCLINACION[/pink]"
            )
            ...
            print(
                Colorate.Horizontal(
                    Colors.red,
                    "[!] PORCENTAJE INVÁLIDO. USA NÚMEROS CON PUNTO O COMA DECIMAL",
                )
            )
            continue
    ...
    custom = IntPrompt.ask(
        "[pink][?]﻿INGRESA PORCENTAJE DE VISIB DE LOS VIDRIOS[/pink]"
    )
    console.print("[red][%] Configurando porcentaje [/red]: ", end=None)
    ...
    print(
        Colorate.Horizontal(Colors.rainbow, "[!] Por favor ingresa la nueva contraseña")
    )
    new_password = prompt_valid_value(
        "[?] Nueva contraseña", "Contraseña", password=True
    )
    ...
    if cpm.change_password(new_password):
        ...
        print(
            Colorate.Horizontal(
                Colors.rainbow,
                "[!] La contraseña debe tener al menos 6 caracteres o hubo un error en la conexión.",
            )
        )
        sleep(2)
        continue
    ...
    print(
        Colorate.Horizontal(
            Colors.rainbow, "[!] Por favor ingresa el nuevo correo electrónico"
        )
    )
    new_email = prompt_valid_value(
        "[?] Nuevo correo electrónico", "Correo electrónico", password=False
    )
    ...
    if cpm.change_email(new_email):
        ...
        print(
            Colorate.Horizontal(
                Colors.rainbow,
                "[!] El correo electrónico no es válido, ya está en uso o hubo un error en la conexión.",
            )
        )
        sleep(2)
        continue
    ...
    source_id = IntPrompt.ask(
        "[bold cyan][?] Ingresa el ID del auto origen[/bold cyan]"
    )
    target_id = IntPrompt.ask(
        "[bold cyan][?] Ingresa el ID del auto destino[/bold cyan]"
    )
    console.print(
        "[bold red][%] Aplicando diseño del auto origen al destino...[/bold red]",
        end=None,
    )
    if cpm.clone_car_design(source_id, target_id):
        print(
            Colorate.Horizontal(
                Colors.rainbow,
                f"✔ El diseño del auto {source_id} fue copiado al auto {target_id} exitosamente.",
            )
        )
        check_exit()
    else:
        print(
            Colorate.Horizontal(
                Colors.red_to_white,
                f"❌ No se pudo copiar el diseño del auto {source_id}.",
            )
        )
        sleep(2)
        continue
    ...
    if resultado.get("ok"):
        print(
            Colorate.Horizontal(Colors.rainbow, "✔ Componentes aplicados correctamente")
        )
    else:
        print(
            Colorate.Horizontal(
                Colors.red_to_white, f"❌ Error: {resultado.get('message')}"
            )
        )
